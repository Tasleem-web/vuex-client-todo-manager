<template>
  <pre> isSubmitted -- {{ isSubmitted }}</pre>
  <div class="login-page mt-5 mb-5">
    <div class="container">
      <div class="row">
        <div class="col-lg-10 offset-lg-1">
          <h3 class="mb-5 text-center">Login Page</h3>
          <div class="bg-white shadow rounded">
            <div class="row">
              <div class="col-md-7 pe-0">
                <div class="form-left h-100 py-5 px-5">
                  <form @submit.prevent="handleSubmit" class="row g-4">
                    <div class="col-12">
                      <label>Username<span class="text-danger">*</span></label>
                      <div class="input-group">
                        <div class="input-group-text">
                          <i class="bi bi-person-fill"></i>
                        </div>
                        <input
                          type="text"
                          class="form-control"
                          placeholder="Enter Username"
                          v-model="user.username"
                        />
                      </div>
                      <p
                        class="text-danger"
                        v-if="isSubmitted && !user.username"
                      >
                        Username is required
                      </p>
                    </div>

                    <div class="col-12">
                      <label>Password<span class="text-danger">*</span></label>
                      <div class="input-group">
                        <div class="input-group-text">
                          <i class="bi bi-person-fill"></i>
                        </div>
                        <input
                          type="password"
                          class="form-control"
                          placeholder="Enter Password"
                          v-model="user.Password"
                        />
                      </div>
                      <p
                        class="text-danger"
                        v-if="isSubmitted && !user.Password"
                      >
                        Password is required
                      </p>
                    </div>
                    <div class="col-sm-6">
                      <div class="form-check">
                        <input
                          class="form-check-input"
                          type="checkbox"
                          id="inlineFormCheck"
                          v-model="user.rememberMe"
                        />
                        <label class="form-check-label" for="inlineFormCheck"
                          >Remember me</label
                        >
                      </div>
                    </div>

                    <!-- <div class="col-sm-6">
                      <a href="#" class="float-end text-primary"
                        >Forgot Password?</a
                      >
                    </div> -->

                    <div class="col-12">
                      <button
                        type="submit"
                        class="btn btn-primary px-4 float-start mt-4"
                      >
                        login
                      </button>
                      <button
                        type="submit"
                        class="btn btn-primary px-4 float-end mt-4"
                      >
                        <router-link :to="{ path: '/sign-up' }" class="text-white">
                          Register
                        </router-link>
                      </button>
                    </div>
                  </form>
                </div>
              </div>
              <div class="col-md-5 ps-0 d-none d-md-block">
                <div
                  class="form-right h-100 bg-primary text-white text-center pt-5"
                >
                  <!-- <i class="bi bi-bootstrap"></i> -->
                  <div>
                    <img
                      src="../assets/shopping-online.jpg"
                      width="200"
                      height="150"
                    />
                  </div>
                  <h2 class="fs-1">Welcome to Online Store</h2>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
export default {
  name: "LoginPage",
  data() {
    return {
      user: {
        username: "",
        password: "",
        rememberMe: true,
      },
      isSubmitted: false,
    };
  },
  methods: {
    handleSubmit() {
      this.isSubmitted = true;
      console.log("this.user", this.user);
    },
  },
};
</script>

<style scoped></style>
